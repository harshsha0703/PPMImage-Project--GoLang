...
func (rect Rectangle) draw(scn screen) (err error) {
	...
	for row := rect.ll.y; row < rect.ur.y; row++ {
		for col := rect.ll.x; col < rect.ur.x; col++ {
...
func (circ Circle) draw(scn screen) (err error) {
	...
	if outOfBounds(Point{circ.center.x - circ.r, circ.center.y - circ.r}, scn) || outOfBounds(Point{circ.center.x + circ.r, circ.center.y + circ.r}, scn) {
		return outOfBoundsErr
	}
...
func (display *Display) clearScreen() {
	// TO DO: Implement this method
	for row := 0; row < display.maxY; row++ {
		for col := 0; col < display.maxX; col++ {
			display.matrix[row][col] = colorMap["white"]
		}
	}
}
...
func (display *Display) initialize(x, y int) {
	...
	for idx := range display.matrix {
		display.matrix[idx] = make([]Color, x)
		for jdx := range display.matrix[idx] {
			display.matrix[idx][jdx] = colorMap["white"]
		}
	}
}
...
func (display *Display) screenShot(f string) (err error) {
	...
	for col := 0; col < display.maxX; col++ {
		for row := 0; row < display.maxY; row++ {
			pixelColor := display.matrix[row][col]
			fmt.Fprintf(file, "%d %d %d ", pixelColor.red, pixelColor.green, pixelColor.blue)
		}
		fmt.Fprintln(file)
	}
	return
